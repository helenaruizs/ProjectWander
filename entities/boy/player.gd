...
@export var inputs: InputController

func _ready() -> void:
	movement.init_setup(self)
	inputs.init_setup(state_machine)

func _physics_process(delta: float) -> void:
		##Flip sprite horizontally
	if velocity.x > 0 :
		visuals.flip_sprite(false)
	elif velocity.x < 0 :
		visuals.flip_sprite(true)

#########################
##### PUBLIC API ########
#########################

func get_move_component() -> MovementComponent:
	return movement

func get_visuals_component() -> VisualsComponent:
	return visuals
